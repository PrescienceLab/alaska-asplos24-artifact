...
def sizeof_fmt(x, pos):
    return '%3.0f' % (x / 1024.0 / 1024.0 / 1024.0)
...
axs.axhline(y=50 * 1024 * 1024 * 1024, linestyle="-", linewidth=1, color="#000000")
...
c = next(color)
load_and_plot('results/redis-alaska-large.csv', 'Anchorage', c, linewidth=3)
# c = next(color)
# load_and_plot('nodefrag.csv', 'Anchorage (No defrag)', c)
c = next(color)
load_and_plot('results/redis-baseline-large.csv', 'Baseline', c, linestyle='--')

# c = next(color)
# load_and_plot('mesh.csv', 'Mesh', c, linestyle=':')
# c = next(color)
# load_and_plot('activedefrag.csv', 'activedefrag', c, linestyle='--')


MB = 1024 * 1024
GB = 1024 * MB
major_ticks = np.arange(0, 150 * GB, 25 * GB)
...
axs.set_xlim(left=0.0, right=45 * 60 * 1000)
...
# plt.tight_layout()
plt.savefig('results/figure11.pdf', bbox_inches='tight', pad_inches=.05)
